...
class OTFInstall
  ...
  def read_otfinstr(path)
    ...
    rg=File.join(fontdir,@otfinstr.instance_variable_get("@regular"))
    bo=File.join(fontdir,@otfinstr.instance_variable_get("@bold"))
    it=File.join(fontdir,@otfinstr.instance_variable_get("@italic"))
    bi=File.join(fontdir,@otfinstr.instance_variable_get("@bolditalic"))
    ...
    @variants.each do |variant|
      ...
      when "default"
        instr=[["",""],["Caps","-fsmcp"]]
        install_font(rg,"",instr)
        install_font(bo,"Bold",instr)
        install_font(it,"Italic",instr)
        install_font(bi,"BoldItalic",instr)
      when "osf"
        instr=[["OsF","-fonum"]]
        install_font(rg,"Regular",instr)
        install_font(bo,"Bold",instr)
        install_font(it,"Italic",instr)
        install_font(bi,"BoldItalic",instr)
